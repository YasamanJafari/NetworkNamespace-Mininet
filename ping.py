...
# def breakFiletoChunks():
# 	chunkID = 1;
# 	f = open(fileName, 'rb')
# 	chunk = f.read(CHUNK_SIZE)
# 	while chunk:
# 		send_one_ping(currSock, fileName + chunk, chunkID)
# 		# print(chunk, chunkID)
# 		# print("*************")
# 		chunkID += 1
# 		chunk = f.read(CHUNK_SIZE)
		...
		for s in readable:
			if s == sys.stdin:
				...
				if cmd == "send":
					...
					chunk = f.read(CHUNK_SIZE)
					while chunk:
						send_time = self.send_one_ping(current_socket, fileName + chunk, chunkID)
						print(chunk, chunkID)
						print("*************\n")
						chunkID += 1
						chunk = f.read(CHUNK_SIZE)
				...
				if receive_time:
					...
					if self.max_time < delay:
						self.max_time = delay
					self.print_success(-1, ip, packet_size, ip_header, icmp_header)
					return -1
				else:
					self.print_failed()
				...
				print("@@@ ")
				print(s) 
				print("Error! : invalid input socket\n")
				return

	# send an ICMP ECHO_REQUEST packet
	def send_one_ping(self, current_socket, msg = "", chunkID = 0):
		
		#Create a new IP packet and set its source and destination IP addresses
		# randomSrc = random.randrange(1, hostsCount)
...
def ping(source, hostname, timeout=1000, count=1, packet_size=55, *args, **kwargs):
...
ping(curr_ip, "10.0.0.2")    #put your IP and destination IP address as the ping function argument and run the code. you can ping
